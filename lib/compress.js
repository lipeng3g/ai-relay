...
// Output: Markdown relay-text string
...
(function (global) {
  ...
  const COMPRESS_FALLBACK = {
    snap_header: '# AICarry Relay Text · from {source} · {date}\n> Mode: {mode} · Original: {msgs} messages ({turns} turns)',
    snap_mode_full: 'Complete Relay',
    snap_mode_smart: 'Concise Relay',
  ...
  function compressFull(session, headerMode = 'full') {
    if (!session?.messages?.length) return '';
    const msgs = session.messages;
    const sourceLabel = PLATFORM_LABEL[session.platform] || session.platform || 'AI';
    const turns = toTurns(msgs);
    const lines = [];
    lines.push(makeHeader(sourceLabel, msgs.length, turns.length, headerMode));
    lines.push('');
  ...
  function compressSmart(session, options) {
    ...
    if (turns.length <= 3) return compressFull(session, 'smart');
  ...
  function estimateStats(session) {
    ...
    const turns = toTurns(session.messages).length;
  ...
  function estimateModeSize(session, mode, options) {
    if (!session?.messages?.length) return { chars: 0, approxTokens: 0 };
    const snap = compress(session, mode, options);
  ...
  function sessionFingerprint(session) {
    ...
    return `${session.platform}:${session.convId || ''}:${session.messages.length}:${(hash >>> 0).toString(16)}`;
  }
